...
abstract class TYModule extends TYComponent
{
	/**
	...
	public function __construct($id,$parent,$config=null)
	{
		...
		Gateway::setPathOfAlias($id,$this->getBasePath());
	...
	public function setBasePath($path)
	{
		...
		throw new ThoughtException("Base path '.$path.' is not a valid directory.");
	...
	public function setModulePath($value)
	{
		...
		throw new ThoughtException("The module path  '.$value.' is not a valid directory.");
	}
	...
	public function setAliases($mappings)
	{
		...
		{
			if(($path=Gateway::getPathOfAlias($alias))!==false)
			Gateway::setPathOfAlias($name,$path);
			else
			Gateway::setPathOfAlias($name,$alias);
		}
	}
	...
	public function getModule($id)
	{
		...
		elseif(isset($this->_moduleConfig[$id]))
		{
			...
			{
				Gateway::trace("Loading \"$id\" module",'system.base.Module');
				$class=$config['class'];
				unset($config['class'], $config['enabled']);
				if($this===Gateway::app())
				$module=Gateway::createComponent($class,$id,null,$config);
				else
				$module=Gateway::createComponent($class,$this->getId().'/'.$id,$this,$config);
	...
	public function setModules($modules)
	{
		foreach($modules as $id=>$module)
		{
			...
			{
				Gateway::setPathOfAlias($id,$this->getModulePath().DIRECTORY_SEPARATOR.$id);
	...
	public function getComponent($id,$createIfNull=true)
	{
		...
		elseif(isset($this->_componentConfig[$id]) && $createIfNull)
		{
			...
			Gateway::log("backtrace",10);
			if(!isset($config['enabled']) || $config['enabled'])
			{
				Gateway::appfootprints("Loading \"$id\" application component",'system.CModule');
				unset($config['enabled']);
				$component=Gateway::createComponent($config);
				$component->init();
